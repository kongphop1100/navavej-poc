    ...
    with st.spinner('กำลังยิง HTTP GET Request ไปยังเซิร์ฟเวอร์ชั้นบน...'):
        try:
            ...
            if response.status_code == 200:
                ...
                customer_type = patient.get('customer_type', 'existing')
                customer_type_th = "ลูกค้าใหม่" if customer_type == "new" else "ลูกค้าเก่า"
                st.sidebar.markdown(f"**สถานะลูกค้า:** {customer_type_th}")
                ...
                if vitals:
                    ...
                    vc5, vc6 = st.sidebar.columns(2)
                    vc5.metric("HbA1c (%)", vitals.get('HbA1c', 'N/A'))
                    vc6.metric("eGFR", vitals.get('eGFR', 'N/A'))

                    vc7, vc8 = st.sidebar.columns(2)
                    vc7.metric("Triglycerides", vitals.get('Triglycerides', 'N/A'))
                    vc8.metric("LDL", vitals.get('LDL', 'N/A'))

                    vc9, _ = st.sidebar.columns(2)
                    vc9.metric("HDL", vitals.get('HDL', 'N/A'))
                ...
                with col2:
                    ...
                    for idx, rec in enumerate(recs, 1):
                        mode_text = "Profile only" if rec.get("recommendation_mode") == "profile_only" else "Hybrid"
                        st.markdown(f"""
                        <div style="padding:15px; border-radius:10px; border:2px solid #5C6BC0; margin-bottom:15px; background-color:#E8EAF6;">
                            ...
                            <span style="color:#3F51B5;">รหัส: {rec['package_id']} | Recommendation score: {(rec['confidence_score'] * 100):.1f}% | โหมด: {mode_text}</span><br/>
                            <span style="color:#455A64;">เหตุผล: {rec.get('reason', 'ไม่มีคำอธิบาย')}</span>
                        </div>
